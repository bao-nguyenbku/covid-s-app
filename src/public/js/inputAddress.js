getDistrict = function () {
    ...
    for (let i = 0; i < hcmAPI.districts.length; i++) {
        const dis = hcmAPI.districts[i];
        if (dis.name == disValue) {
            let wards = document.getElementById('wards');
            while (wards.lastChild && wards.childElementCount > 1) {
                wards.lastChild.remove();
            }
            for (let j = 0; j < dis.wards.length; j++) {
                let option = document.createElement('option');
                option.setAttribute('value', dis.wards[j].name);
                option.textContent = dis.wards[j].name;
